...
function App() {
  ...
  const searchLocation = () => {
    ...
    let url =
      ...
      input +
      "&limit=5&appid=5e532fb5655cf4c4f0e3d2a54be74067";
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
  ...
  useEffect(() => {
    fetch(
      "https://api.openweathermap.org/data/2.5/weather?lat=" +
        ...
        "&lon=" +
        coord.long +
        "&appid=5e532fb5655cf4c4f0e3d2a54be74067"
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  return weather === [1, 2, 3, 4] || currentWeather === null ? (
    <div />
  ) : (
    <div className="App">
      <div className={"App-header"}>
        <div className="input">
          ...
          />
          <button className="Search" onClick={() => searchLocation()}>
      ...
      <div className="main">
        <div className="stats">
          ...
          <h1 className="currentTemp">
            {toggle
              ? (parseFloat(currentWeather.main.temp) - 273).toFixed(0)
              : (
                  ((parseFloat(currentWeather.main.temp) - 273) * 9) / 5 +
                  32
                ).toFixed(0)}
            {unit}
          </h1>
        ...
        <div className="list">
          <Weather
            temp={
              toggle
                ? (parseFloat(weather[0].temp) - 273).toFixed(0)
                : (((parseFloat(weather[0].temp) - 273) * 9) / 5 + 32).toFixed(
                    0
                  )
            }
            time={weather[0].time}
          ...
          <Weather
            temp={
              toggle
                ? (parseFloat(weather[1].temp) - 273).toFixed(0)
                : (((parseFloat(weather[1].temp) - 273) * 9) / 5 + 32).toFixed(
                    0
                  )
            }
            time={weather[1].time}
          ...
          <Weather
            temp={
              toggle
                ? (parseFloat(weather[2].temp) - 273).toFixed(0)
                : (((parseFloat(weather[2].temp) - 273) * 9) / 5 + 32).toFixed(
                    0
                  )
            }
            time={weather[2].time}
          ...
          <Weather
            temp={
              toggle
                ? (parseFloat(weather[3].temp) - 273).toFixed(0)
                : (((parseFloat(weather[3].temp) - 273) * 9) / 5 + 32).toFixed(
                    0
                  )
            }
            time={weather[3].time}
        ...
        <div className="current-container">
          <div className={"current-attributes"}>
            ...
            <p className={"attributes"}>
              {toggle
                ? (parseFloat(currentWeather.main.feels_like) - 273).toFixed(0)
                : (
                    ((parseFloat(currentWeather.main.feels_like) - 273) * 9) /
                      5 +
                    32
                  ).toFixed(0)}
              {unit}
          ...
          <div className={"current-attributes"}>
            ...
            <p className="attributes">
              {(parseFloat(currentWeather.wind.speed) * 1.852).toFixed(0)} km/h
            </p>
          </div>
          <div className={"current-attributes"}>
